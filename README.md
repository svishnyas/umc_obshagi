...
- База очищается от тестовых постов/комментариев/лайков/событий.
- Создаётся только один владелец: **`Артём Родионов`**, **комн. `301`**, Волхонка.
- Пароль владельца по умолчанию: `demo123` (можно переопределить переменной `SEED_OWNER_PASSWORD`).
...
Опционально первый контент: `npm run db:seed` (очищает базу и создаёт только стартового владельца).
